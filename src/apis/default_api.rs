...
/// struct for typed errors of method [`wallets_api_v1_wallet_balance_retrieve`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum WalletsApiV1WalletBalanceRetrieveError {
    UnknownValue(serde_json::Value),
}
...
pub async fn wallets_api_v1_wallet_balance_retrieve(configuration: &configuration::Configuration, ) -> Result<models::WalletBalance, Error<WalletsApiV1WalletBalanceRetrieveError>> {

    let uri_str = format!("{}/wallets/api/v1/wallet-balance/", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::WalletBalance`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::WalletBalance`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<WalletsApiV1WalletBalanceRetrieveError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
